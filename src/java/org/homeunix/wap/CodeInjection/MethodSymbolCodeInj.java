 ...
 */
public class MethodSymbolCodeInj extends MethodSymbol{
 
    
    // ****** CONSTRUCTORS ***********
     //public MethodSymbolCodeInj(String function_name, int line, int tainted, Scope parent, String filename) {
     public MethodSymbolCodeInj(MethodSymbol mtsym) {
    ...
    // *** METHODS ********     
    public int analyseMethodCodeInj(MethodSymbol meth_sym,int line, Scope scp, TaintedTable mts_princ, UntaintedTable mus_princ, String filename, String file_methClass, Instance inst, Map mct, String typeMeth) throws IOException{
        SymbolTable t = GlobalDataCodeInj.MainSymbolTable.get(scp.getScopeSymbol().getFileSymbol());;
        TaintedTable tt = null;
        UntaintedTable ut = null;
        InstanceTable instTable = GlobalDataCodeInj.MainInstancesTable.get(scp.getScopeSymbol().getFileSymbol());
        SymbolTable st = GlobalDataCodeInj.MainSymbolTable.get(scp.getScopeSymbol().getFileSymbol());
        
        if (t == null){
            t = GlobalDataCodeInj.MainSymbolTable.get(filename);
            st = GlobalDataCodeInj.MainSymbolTable.get(filename);
        }
        ...
        if (inst != null){
            ...
            st = GlobalDataCodeInj.MainSymbolTable.get(inst.getFileInstance());
            instTable = GlobalDataCodeInj.MainInstancesTable.get(inst.getFileInstance());
        }
        //else{
        //    t = GlobalDataSqli.MainSymbolTable.get(scp.getScopeSymbol().getFileSymbol());
        //}
	...
	for(; it.hasNext();){
            ...
            if (!cs.getMembers().isEmpty()){
                ...
                if (cs.getScopeSymbol().getIsClassInstruction() == true || t.containsSymbol(cs.getScopeName()) == true){
                    ...
                    css.resolveSymbolClassCodeInj(meth_sym_aux, cs, mts_aux, mus_aux, filename, mtsc, inst, mct, true, null, null, typeMeth, isClassInstruct);
                }
                else{
                    // include
                    if (cs.getScopeName().equals("include") == true){
                              FuncAndIncCodeInj css = new FuncAndIncCodeInj(cs);
                            css.resolveSymbolIncludeCodeInj(cs, mts_aux, mus_aux, GlobalDataCodeInj.MainIncludeFilesTable, filename,
                                                            GlobalDataCodeInj.MainFunctionsTable, GlobalDataCodeInj.MainSymbolTable,
                        ...
                        // instrucao normal
                        FuncAndIncCodeInj css = new FuncAndIncCodeInj(cs);
                        css.resolveSymbolFunctionCodeInj(meth_sym_aux, cs, mts_aux, mus_aux, GlobalDataCodeInj.MainIncludeFilesTable, filename,
                                                        GlobalDataCodeInj.MainFunctionsTable, GlobalDataCodeInj.MainSymbolTable,
                                                        GlobalDataCodeInj.MainFunctionsTaintedTable, GlobalDataCodeInj.MainLinesToCorrect);
                    }
                }
	...
	if (taints_returns > 0){
            ...
            if (typeMeth.equals("constructor") == true || scp.getScopeSymbol().getName().equals(inst.getInstanceName()) == true)
    ...
    public String analyseUserFunctionCodeInj(MethodSymbol meth_sym, int line, Scope scp, TaintedTable mts_princ, UntaintedTable mus_princ, String filename, String file_meth, Map mift, Map mft, Map mst, Map mftt, Map MainLinesToCorrect) throws IOException{
        ...
        if (GlobalDataCodeInj.MainFunctionsTaintedTable.containsKey(filename) == true)
            mtt = (MethodTaintedTable) GlobalDataCodeInj.MainFunctionsTaintedTable.get(filename);
        else{
            mtt = new MethodTaintedTable(filename);
            GlobalDataCodeInj.MainFunctionsTaintedTable.put(filename, mtt);
        }
	...
	UntaintedTable mus_aux = new UntaintedTable();
        ...
        if (GlobalDataCodeInj.MainFunctionsCallsTable.containsKey(filename) == true)
            mtc = GlobalDataCodeInj.MainFunctionsCallsTable.get(filename);
        else{
            mtc = new MethodTableCalls(filename);
            GlobalDataCodeInj.MainFunctionsCallsTable.put(filename, mtc);
        }
	...
	it = meth_sym_aux.getMembers().iterator();
	Scope cs;
        //FuncAndIncCodeInj cs;
	for(; it.hasNext();){
            cs = (Scope) it.next();
            FuncAndIncCodeInj css = new FuncAndIncCodeInj(cs);
            if (!cs.getMembers().isEmpty()){
                
		css.resolveSymbolFunctionCodeInj(meth_sym_aux, cs, mts_aux, mus_aux, mift, filename, mft, mst, mftt, MainLinesToCorrect);
		// caso o return seja tainted incrementar os returns tainted
		Scope cs_aux = cs.getScopeSymbol().getRootScope();
